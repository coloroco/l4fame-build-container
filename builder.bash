...
git clone https://github.com/FabricAttachedMemory/nvml.git || \
    ( cd nvml && git checkout upstream && fix_nvml_rules && set -- `git pull` && [ "$1" == "Updating" ] ) && \
...
git clone https://github.com/keith-packard/tm-manifesting.git || \
    ( cd tm-manifesting && set -- `git pull` && [ "$1" == "Updating" ] ) && \
...
git clone https://github.com/FabricAttachedMemory/tm-libfuse.git || \
    ( cd tm-libfuse && set -- `git pull` && [ "$1" == "Updating" ] ) && \
    ( cd tm-libfuse && run_update && gbp buildpackage );
...
git clone https://github.com/FabricAttachedMemory/libfam-atomic.git || \
    ...
    ( cd libfam-atomic && run_update && gbp buildpackage --git-upstream-tree=branch );
